...
void MainWindow::getPluginsList()
{
    ...
    foreach(QFileInfo filter, filters)
    {
        if (QLibrary::isLibrary(filter.absoluteFilePath()))
        {
            ...
            {
                QMessageBox::warning(this, tr("Warning"), QString(tr("Make sure %1 is a correct"
        ...
        else
        {
            QMessageBox::warning(this, tr("Warning"),
                                 QString(tr("Make sure onlu pluging"
...
void MainWindow::on_helpButton_pressed()
{
    if (ui->filtersList->currentRow() >= 0)
    {
        QPluginLoader pluginLoader(
                    ...
                    FILTERS_SUBFOLDER + ui->filtersList->currentItem()->text());
        CVPluginInterface *plugin = dynamic_cast<CVPluginInterface*>(
